/**
 * blog_posts 단건 INSERT — 'korean-restaurant-ordering-guide' (Life in Korea 2/3)
 *
 ...
 *
 * 실행: npx tsx scripts/insert-post-korean-restaurant-ordering-guide.ts
 */
...
const MD_PATH = path.resolve(process.cwd(), 'scripts', '_post-korean-restaurant-ordering-guide.md')
...
const POST = {
  ...
  tags:         ['Korean Dining', 'Restaurant Etiquette', 'Seoul Travel', 'Korea Culture'],
  title:        "Don't Wait for Eye Contact: How to Call Staff and Navigate Korean Restaurants",
  description:  'Master table call buttons, tucked-away utensil drawers, and self-service stations to navigate dining in South Korea without unnecessary delays.',
  content:      fs.readFileSync(MD_PATH, 'utf8').trimEnd(),
  published_at: '2026-09-21T02:00:00+00:00',   // 1/3 카페편(01:00)보다 뒤
  is_paused:    false,
}
...
async function main() {
  ...
  const PREV = '2026-09-21T01:00:00+00:00'   // 직전 글 korean-cafe-laptop-safety-rules
  ...
  console.log(`3. ${tables === 1 && cells === 15 ? '✅' : '❌'} 표 ${tables}개 / 셀 ${cells}개 (기대 1 / 15)`)
