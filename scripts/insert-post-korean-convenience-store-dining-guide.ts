/**
 * blog_posts 단건 INSERT — 'korean-convenience-store-dining-guide' (Life in Korea 3/3)
 *
 ...
 *
 * 실행: npx tsx scripts/insert-post-korean-convenience-store-dining-guide.ts
 */
...
const MD_PATH = path.resolve(process.cwd(), 'scripts', '_post-korean-convenience-store-dining-guide.md')
...
const POST = {
  slug:         'korean-convenience-store-dining-guide',
  locale:       'en',
  app:          'k-patto',
  category:     'Korean Culture',
  tags:         ['Korean Convenience Store', 'Seoul Food Guide', 'Budget Travel', 'K-Food Combinations'],
  title:        'More Than Just Snacks: A Practical Guide to Eating at Korean Convenience Stores',
  description:  'Master proven instant food pairings, appliance protocols, and proper disposal etiquette to eat affordably at Korean convenience stores.',
  content:      fs.readFileSync(MD_PATH, 'utf8').trimEnd(),
  // 03:00 은 아직 오지 않은 시각이라 쓸 수 없다(미래면 목록에서 빠진다).
  // 2분 전으로 잡아 '과거이면서 직전 글(02:00)보다 뒤'를 동시에 만족시킨다.
...
async function main() {
  ...
  const PREV = '2026-09-21T02:00:00+00:00'   // 직전 글 korean-restaurant-ordering-guide
  ...
  console.log(`3. ${tables === 1 && cells === 9 ? '✅' : '❌'} 표 ${tables}개 / 셀 ${cells}개 (기대 1 / 9)`)
