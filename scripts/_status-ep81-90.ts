...
async function main() {
  const { data: eps } = await sb.from('kp_episodes').select('id,episode_num').gte('episode_num',81).lte('episode_num',90).order('episode_num')
  if (!eps?.length) { console.log('EP81~90 없음'); return }
