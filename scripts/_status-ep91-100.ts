...
async function main() {
  const { data: eps } = await sb.from('kp_episodes').select('id,episode_num').gte('episode_num',91).lte('episode_num',100).order('episode_num')
  if (!eps?.length) { console.log('EP91~100 없음'); return }
  ...
  console.log(`-----|------|------|------\n합계 |  ${String(tAll).padStart(3)} |  ${String(tDone).padStart(3)} |  ${String(tPend).padStart(3)}`)
  console.log(tPend <= 85 ? `\n✅ 한도(85) 이내 — 전부 처리 가능 (${tPend}건)` : `\n⚠️  ${tPend}건 중 오늘은 85건만 처리`)
}
