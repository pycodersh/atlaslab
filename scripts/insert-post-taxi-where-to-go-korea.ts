/**
 * blog_posts 단건 INSERT — 'taxi-where-to-go-korea' (여행 회화 12번 · 교통 첫 글)
 *
 ...
 * 영상 J72LDTaIBv0 는 oEmbed 로 사전 확인했다
 * ("Driver, Myeongdong please. STEP ON IT.", K-PATTO 채널).
 *
 * category: 교통 주제지만 새 값을 만들지 않는다. 편의점 글의 'Real-Life Korean'
 * 은 편의점 전용 이름이 아니라 여행 회화 전체를 담는 값이고(공개 30편,
 * Korean phrases 탭), DB 에 교통 전용 값은 없다 — _precheck-post.ts 로 확인.
 *
 ...
 *
 * 실행: npx tsx scripts/insert-post-taxi-where-to-go-korea.ts
 */
...
const MD_PATH = path.resolve(process.cwd(), 'scripts', '_post-taxi-where-to-go-korea.md')
...
const POST = {
  slug:         'taxi-where-to-go-korea',
  locale:       'en',
  app:          'k-patto',
  category:     'Real-Life Korean',
  tags:         ['taxi', 'transportation', 'travel', 'korean phrases'],
  title:        'Getting into a Seoul taxi and telling the driver where to go',
  description:  'What the driver asks when you get in, how to say your destination, and what to do when you cannot pronounce it.',
...
async function main() {
  ...
  const PREV = '2026-09-19T00:00:00+00:00'   // 22번 show-id-buying-alcohol
  if (new Date(POST.published_at).getTime() <= new Date(PREV).getTime()) {
    console.error(`⛔ published_at 이 22번(${PREV})보다 앞 — 중단`); process.exit(1)
  }
  ...
  console.log(`2. ${html.includes('youtube.com/embed/J72LDTaIBv0') ? '✅' : '❌'} YouTube 임베드`)
