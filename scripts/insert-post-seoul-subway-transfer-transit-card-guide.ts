/**
 * blog_posts 단건 INSERT — 'seoul-subway-transfer-transit-card-guide' (Life in Korea)
 *
 ...
 * 표지와 본문 그림 2장은 scripts/gen_subway_illustrations.py 로 그렸다
 * (표지는 COVER_BY_SLUG, 본문 그림은 public/images/posts). 그림이 배포된 뒤 실행한다.
 *
 ...
 *
 * 실행: npx tsx scripts/insert-post-seoul-subway-transfer-transit-card-guide.ts
 */
...
const MD_PATH = path.resolve(process.cwd(), 'scripts', '_post-seoul-subway-transfer-transit-card-guide.md')
...
const POST = {
  slug:         'seoul-subway-transfer-transit-card-guide',
  locale:       'en',
  app:          'k-patto',
  category:     'Korean Culture',
  tags:         ['Seoul Subway', 'T-money', 'Climate Card', 'Korea Transit'],
  title:        "Never Buy Single Tickets: How to Master Seoul's Subway Transfers and Transit Cards",
  description:  "Master Seoul's 30-minute transit transfer discounts, recover from wrong-direction fare gates for free, and choose between T-money and the Climate Card.",
  content:      fs.readFileSync(MD_PATH, 'utf8').trimEnd(),
  // 삽입 2분 전 — 과거이면서 직전 글(택시 가이드 09-22 01:36)보다 뒤
...
async function main() {
  ...
  const PREV = '2026-09-22T01:36:25+00:00'   // 직전 글 korea-taxi-survival-guide
  ...
  const imgs = ['subway-transfer-30min-window.png', 'subway-wrong-gate-fix.png']
  const imgOk = await Promise.all(imgs.map(async f => (await fetch(`${PROD}/images/posts/${f}`)).status === 200))
  console.log(`2. ${imgs.every(f => html.includes(f)) && imgOk.every(Boolean) ? '✅' : '❌'} 본문 그림 2장 (참조 + 파일 200)`)
  const tables = (html.match(/<table/g) ?? []).length
  const cells = (html.match(/<td/g) ?? []).length
  console.log(`3. ${tables === 1 && cells === 16 ? '✅' : '❌'} 표 ${tables}개 / 셀 ${cells}개 (기대 1 / 16)`)
