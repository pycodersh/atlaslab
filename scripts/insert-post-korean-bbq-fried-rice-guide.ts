/**
 * blog_posts 단건 INSERT — 'korean-bbq-fried-rice-guide' (Korean food)
 *
 ...
 * 둘뿐이다. 식탁 위에서 볶음밥을 만드는 법이라 Cooking Basics(쌈 가이드와 같음).
 * 원고의 category:"culture" 는 쓰지 않는다.
 * 표지(04)는 COVER_BY_SLUG, 본문 이미지(05) 1장은 public/images/posts 에 있다.
 ...
 *
 * 실행: npx tsx scripts/insert-post-korean-bbq-fried-rice-guide.ts
 */
...
const MD_PATH = path.resolve(process.cwd(), 'scripts', '_post-korean-bbq-fried-rice-guide.md')
...
const POST = {
  slug:         'korean-bbq-fried-rice-guide',
  locale:       'en',
  app:          'k-pantry',
  category:     'Cooking Basics',
  tags:         ['Korean BBQ', 'Fried Rice', 'Bokkeumbap', 'Seoul Dining Guide'],
  title:        'The Science of Korean BBQ Fried Rice: 3 Rules of the Tabletop Finale',
  description:  'Why fried rice functions as the true savory dessert at Korean BBQ, the emulsification physics behind it, and a 3-step guide to scoring optimal nurungji.',
  content:      fs.readFileSync(MD_PATH, 'utf8').trimEnd(),
  // 삽입 2분 전 — 과거이면서 직전 글(쌈 가이드 11:20)보다 뒤
...
async function main() {
  ...
  const PREV = '2026-09-21T11:20:25+00:00'   // 직전 글 korean-ssambap-dining-guide
  ...
  const imgs = ['bbq-fried-rice-pressed.jpg']
  const imgOk = await Promise.all(imgs.map(async f => (await fetch(`${PROD}/images/posts/${f}`)).status === 200))
  console.log(`2. ${imgs.every(f => html.includes(f)) && imgOk.every(Boolean) ? '✅' : '❌'} 본문 이미지 1장 (참조 + 파일 200)`)
