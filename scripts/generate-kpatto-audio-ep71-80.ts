/**
 * K-PATTO EP71~80 음성 생성 (에러 즉시 중단 버전)
 *
 * ⚠️ 중단 규칙
 *  - TTS API 에러, Storage 업로드 실패, DB 갱신 실패, 0바이트 파일 → 즉시 중단
 *  - 에러를 삼키고 계속 진행하는 try/catch-continue 없음
 *  - 중단 시 보고: 마지막 성공 항목, 실패 항목 대사, 오류 전문, 성공 총 건수
 *
 * 실행: npx tsx scripts/generate-kpatto-audio-ep71-80.ts
 */
...
async function main() {
  const EP_FROM = 71, EP_TO = 80, DAILY_LIMIT = 85
  ...
  if (!episodes?.length) throw new Error('EP71~80 에피소드를 찾을 수 없습니다')

  const epIds = episodes.map(e => e.id)
  const epNumMap = new Map(episodes.map(e => [e.id, e.episode_num]))

  // 2. 미생성 버블 조회 (audio_url = null 만)
