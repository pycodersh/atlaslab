/**
 * K-PATTO EP61~70 음성 생성 (에러 즉시 중단 버전)
 *
 * ⚠️ 중단 규칙
 ...
 *
 * 실행: npx tsx scripts/generate-kpatto-audio-ep61-70.ts
 */
...
async function main() {
  const EP_FROM = 61, EP_TO = 70, DAILY_LIMIT = 85
  ...
  if (!episodes?.length) throw new Error('EP61~70 에피소드를 찾을 수 없습니다')
