/**
 * Story translation batch 2 — Stories 26-50
 * Translates subtitle, storyNote, and paragraph text (from English) into 6 languages.
 * Output: data/story-translations-batch2.ts
 *
 * Run: npx tsx scripts/translate-stories-batch2.ts
 */
...
function buildPrompt(story: (typeof magazineStories)[0]): string {
  ...
  return `You are translating content for an English learning app. Learners use these translations to understand English text in their native language.
...
Subtitle (Korean original, translate into each target language): "${story.subtitleKo}"
Story note (Korean original, translate into each target language): "${story.storyNote}"
...
Return ONLY a valid JSON object with this exact structure (no markdown, no extra text):
{
  ...
  "ja": { "subtitle": "...", "storyNote": "...", "paragraphs": [${story.paragraphs.map(p => `{"id":"${p.id}","translation":"..."}`).join(',')}] },
  "zh-CN": { "subtitle": "...", "storyNote": "...", "paragraphs": [${story.paragraphs.map(p => `{"id":"${p.id}","translation":"..."}`).join(',')}] },
  "zh-TW": { "subtitle": "...", "storyNote": "...", "paragraphs": [${story.paragraphs.map(p => `{"id":"${p.id}","translation":"..."}`).join(',')}] },
  "fr": { "subtitle": "...", "storyNote": "...", "paragraphs": [${story.paragraphs.map(p => `{"id":"${p.id}","translation":"..."}`).join(',')}] },
  "de": { "subtitle": "...", "storyNote": "...", "paragraphs": [${story.paragraphs.map(p => `{"id":"${p.id}","translation":"..."}`).join(',')}] }
}`
}
...
// ── Main ──────────────────────────────────────────────────────────────────────
async function main() {
  const batch = magazineStories.filter(s => s.id >= 26 && s.id <= 50)
  console.log(`Translating ${batch.length} stories (26–50) to ${LANGS.length} languages…\n`)
  ...
  for (const story of batch) {
    ...
    if (story.id < 50) await sleep(800)
  }

  // ── Write output file ─────────────────────────────────────────────────────
  const outPath = path.resolve(process.cwd(), 'data/story-translations-batch2.ts')
  const fileContent = `/**
 * Story translations — Batch 2 (Stories 26–50)
 * Auto-generated by scripts/translate-stories-batch2.ts
...
export const storyTranslationsBatch2: StoryTranslation[] = ${JSON.stringify(results, null, 2)}
`
  fs.writeFileSync(outPath, fileContent, 'utf-8')
  console.log(`\nOutput written to data/story-translations-batch2.ts`)
