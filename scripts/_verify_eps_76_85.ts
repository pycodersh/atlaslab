...
async function main() {
  ...
  console.log('\n=== 4. 청취 샘플 (EP75과 이어지는지 대조) ===')
  const { data: ep70 } = await sb.from('kp_dialogues')
    .select('id, speaker, text_ko, audio_url').eq('episode_id', 75).not('audio_url', 'is', null).order('order_num').limit(2)
  for (const d of ep70 ?? []) console.log(`  [EP75 기존] [${d.speaker}] "${d.text_ko}"\n     ${d.audio_url}`)
  for (const ep of EPS) {
