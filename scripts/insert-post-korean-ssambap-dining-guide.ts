/**
 * blog_posts 단건 INSERT — 'korean-ssambap-dining-guide' (Korean food)
 *
 ...
 * 둘뿐이다. 쌈을 싸고 먹는 법이 중심이라 Cooking Basics — 기존
 * "What Is Ssambap?" 글도 여기 있다. 원고의 category:"culture" 는 쓰지 않는다.
 * 표지는 COVER_BY_SLUG, 본문 이미지 2장은 public/images/posts 에 있다.
 ...
 *
 * 실행: npx tsx scripts/insert-post-korean-ssambap-dining-guide.ts
 */
...
const MD_PATH = path.resolve(process.cwd(), 'scripts', '_post-korean-ssambap-dining-guide.md')
...
const POST = {
  slug:         'korean-ssambap-dining-guide',
  locale:       'en',
  app:          'k-pantry',
  category:     'Cooking Basics',
  tags:         ['Korean Food', 'Ssam Guide', 'Dining Etiquette', 'Seoul Travel'],
  title:        'The Korean Ssam Guide: Leaf Combinations and the One-Bite Rule',
  description:  'Master the art of Korean leaf wraps, from lettuce and perilla pairings to proper portioning and the essential one-bite dining protocol.',
  content:      fs.readFileSync(MD_PATH, 'utf8').trimEnd(),
  // 삽입 2분 전 — 과거이면서 직전 글(편의점 식사 02:11)보다 뒤
...
async function main() {
  ...
  const PREV = '2026-09-21T02:11:50+00:00'   // 직전 글 korean-convenience-store-dining-guide
  ...
  const imgs = ['ssam-greens-platter.jpg', 'ssam-assembly-chopsticks.jpg']
