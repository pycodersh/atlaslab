/**
 * K-PATTO EP91~100 음성 생성 (에러 즉시 중단 버전)
 *
 * ⚠️ 중단 규칙
 ...
 *
 * 실행: npx tsx scripts/generate-kpatto-audio-ep91-100.ts
 */
...
async function main() {
  const EP_FROM = 91, EP_TO = 100, DAILY_LIMIT = 85
  ...
  if (!episodes?.length) throw new Error('EP91~100 에피소드를 찾을 수 없습니다')
  ...
  for (const ep of episodes) {
    ...
    console.log(`EP${String(ep.episode_num).padStart(3,'0')} |   ${String(s.ok).padStart(2)} |   ${String(fail).padStart(2)} |   ${String(s.total).padStart(2)}`)
  }
