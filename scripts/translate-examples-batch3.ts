/**
 * Pattern Example translations — Batch 3 (Stories 41–100, Patterns pt41-1 ~ pt100-5)
 ...
 * Output: data/pattern-example-translations-batch3.ts
 * Run: npx tsx scripts/translate-examples-batch3.ts
 */
...
// ── batch 1 scope: stories 1–20 → pt41-1 ~ pt100-5 ────────────────────────────
const BATCH_IDS = Array.from({ length: 60 }, (_, s) =>
  Array.from({ length: 5 }, (_, p) => `pt${s + 41}-${p + 1}`)
).flat()
...
// ── main ──────────────────────────────────────────────────────────────────────
async function main() {
  console.log(`Batch 3: ${batchPatterns.length} patterns (pt41-1 ~ pt100-5), 5 examples each`)
  ...
  const outPath = path.resolve(process.cwd(), 'data/pattern-example-translations-batch3.ts')
  fs.writeFileSync(outPath, `/**
 * Pattern Example translations — Batch 3 (Stories 41–100, pt41-1 ~ pt100-5)
 * 100 patterns × 5 examples × 6 languages (es/ja/zh-CN/zh-TW/fr/de)
...
export const patternExampleTranslationsBatch3: PatternExampleTranslation[] = ${JSON.stringify(output, null, 2)}
`, 'utf-8')
  ...
  // ── Validation ───────────────────────────────────────────────────────────
  console.log('\n══ Validation ══════════════════════════════════════════════')
  console.log(`Pattern count  : ${output.length} / 300 ${output.length === 300 ? '✓' : '✗'}`)
  ...
  for (const lang of LANGS) {
    ...
    console.log(`${lang.padEnd(6)}         : ${filled} / 300 ${filled === 300 ? '✓' : '✗'}`)
  }
  ...
  const ok = output.length === 300 && wrongCount.length === 0 && empties === 0 && errors.length === 0
