/**
 * Pattern Example translations — Batch 1 (Stories 1–20, Patterns pt1-1 ~ pt20-5)
 ...
 * Output: data/pattern-example-translations-batch1.ts
 * Run: npx tsx scripts/translate-examples-batch1.ts
 */
...
// ── batch 1 scope: stories 1–20 → pt1-1 ~ pt20-5 ────────────────────────────
const BATCH_IDS = Array.from({ length: 20 }, (_, s) =>
  Array.from({ length: 5 }, (_, p) => `pt${s + 1}-${p + 1}`)
).flat()
...
// ── main ──────────────────────────────────────────────────────────────────────
async function main() {
  console.log(`Batch 1: ${batchPatterns.length} patterns (pt1-1 ~ pt20-5), 5 examples each`)
  ...
  const outPath = path.resolve(process.cwd(), 'data/pattern-example-translations-batch1.ts')
  fs.writeFileSync(outPath, `/**
 * Pattern Example translations — Batch 1 (Stories 1–20, pt1-1 ~ pt20-5)
 * 100 patterns × 5 examples × 6 languages (es/ja/zh-CN/zh-TW/fr/de)
...
export const patternExampleTranslationsBatch1: PatternExampleTranslation[] = ${JSON.stringify(output, null, 2)}
`, 'utf-8')
