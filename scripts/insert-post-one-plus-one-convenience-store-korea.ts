/**
 * blog_posts 단건 INSERT — 'one-plus-one-convenience-store-korea' (여행 회화 21번)
 *
 * app / locale / category / tags 형식은 기존 편의점 글 두 편
 * (need-a-bag-korean-checkout, cup-ramen-convenience-store-korea)을 조회해 그대로 따른다.
 ...
 * 영상 MXaqcNgF-RI 는 oEmbed 로 사전 확인했다
 * ("What \"1+1\" Means at a Korean Convenience Store", K-PATTO 채널).
 *
 * published_at 은 오늘 00:00 UTC 가 이미 지났으면 그 값, 아니면 어제 00:00 UTC.
 * (미래 시각이면 목록·사이트맵의 lte(now) 조건에서 빠진다)
 *
 * 실행: npx tsx scripts/insert-post-one-plus-one-convenience-store-korea.ts
 */
...
const MD_PATH = path.resolve(process.cwd(), 'scripts', '_post-one-plus-one-convenience-store-korea.md')
...
const POST = {
  slug:         'one-plus-one-convenience-store-korea',
  locale:       'en',
  app:          'k-patto',
  category:     'Real-Life Korean',
  tags:         ['convenience store', '1+1', 'travel', 'korean phrases'],
  title:        'Buy one, get one free at Korean convenience stores',
  description:  'Why the staff tells you to grab another drink, and how 1+1 and 2+1 deals work at Korean convenience stores.',
...
async function main() {
  ...
  console.log(`2. ${html.includes('youtube.com/embed/MXaqcNgF-RI') ? '✅' : '❌'} YouTube 임베드`)
