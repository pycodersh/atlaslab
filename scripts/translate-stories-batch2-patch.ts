/**
 * Patch script — retranslate failed stories from batch 2
 * Failed: 26, 31, 33, 34, 35, 37, 39, 41, 43, 46, 49
 */
...
const FAILED_IDS = [26, 31, 33, 34, 35, 37, 39, 41, 43, 46, 49]
...
async function translateStory(story: (typeof magazineStories)[0]): Promise<StoryTranslation> {
  ...
  try {
    ...
    const cleaned = raw.replace(/:\s*"((?:[^"\\]|\\.)*)"/g, (_match, inner: string) => {
 ...
 * Auto-generated by scripts/translate-stories-batch2.ts + patch
...
`
  ...
  console.log('\n── Validation ───────────────────────────────────────────────')
  console.log(`Story IDs      : ${merged.map(r => r.storyId).join(', ')}`)
