...
interface Props {
  /** 정규화된 에피소드 id (kp-ep-NNN). 서버 컴포넌트가 존재·권한 검증을 마친 뒤 전달한다. */
  id: string
}
...
export function KPattoStoryClient({ id }: Props) {
  const { prefs } = usePreferences()
  const ui = getUI(prefs.language)
  ...
  // id 형식·존재 여부·구독 권한은 서버 컴포넌트(page.tsx)에서 이미 검증됐다.
  const epNum = story?.episode ?? parseInt(id.match(/kp-ep-(\d+)/)?.[1] ?? '0')
  ...
  // EP10 + non-subscriber → show "Unlock EP11".
  // EP11+ URL 은 비구독자에게 서버에서 404 이므로 구독 페이지로 직접 보낸다.
