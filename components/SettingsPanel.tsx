...
export function SettingsPanel() {
  ...
  const ageOptions: Array<{ label: string; value: AgeGroup }> = [
    { label: t('age_elementary'), value: "elementary" },
    { label: t('age_middle'),     value: "middle" },
    { label: t('age_high'),       value: "high" },
    { label: t('age_worker'),     value: "worker" },
  ];

  const interestOptions: Array<{ label: string; value: InterestArea }> = [
    { label: t('interest_daily'),       value: "daily" },
    { label: t('interest_investment'),  value: "investment" },
    { label: t('interest_business'),    value: "business" },
    { label: t('interest_travel'),      value: "travel" },
    { label: t('interest_game'),        value: "game" },
    { label: t('interest_it'),          value: "it" },
  ];

  const difficultyOptions: Array<{ label: string; sub: string; value: Difficulty }> = [
    { label: "Normal",   sub: t('difficulty_basic'),    value: "normal"   },
    { label: "Advanced", sub: t('difficulty_practical'), value: "advanced" },
    { label: "Native",   sub: t('difficulty_native'),   value: "native"   },
  ];
    ...
    <AppShell>
      <div className="space-y-6">
        <header className="space-y-1 pt-1">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-[#1F2937]">{t('settings_title')}</h1>
        </header>
        ...
        <Card className="border-[#E8F0FE] shadow-none">
          <CardContent className="p-5">
            <section className="space-y-3">
              <div>
                <h2 className="text-base font-bold text-[#1F2937]">{t('difficulty_label')}</h2>
                <p className="mt-0.5 text-xs text-[#9EAEC8]">{t('difficulty_desc')}</p>
              </div>
        ...
        <Card className="border-[#E8F0FE] shadow-none">
          <CardContent className="space-y-5 p-5">
            <section className="space-y-3">
              <h2 className="text-base font-bold text-[#1F2937]">{t('level_label')}</h2>
            ...
            <section className="space-y-3">
              <h2 className="text-base font-bold text-[#1F2937]">{t('interest_label')}</h2>
              <div className="grid grid-cols-3 gap-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-2xl bg-[#F5F8FF] p-4">
                <p className="text-xs font-semibold text-[#9EAEC8]">{t('speech_rate')}</p>
                <p className="mt-1 text-2xl font-bold text-[#1F2937]">
                  {settings.dailyStoryCount}
                </p>
              </div>
              <div className="rounded-2xl bg-[#F5F8FF] p-4">
                <p className="text-xs font-semibold text-[#9EAEC8]">{t('voice')}</p>
                <p className="mt-1 text-2xl font-bold text-[#1F2937]">{t('difficulty_basic')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
