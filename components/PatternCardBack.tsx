...
export function PatternCardBack({
  ...
  return (
    <div className="absolute inset-0 flex flex-col rounded-[28px] border border-[#E8F0FE] bg-white px-5 pb-5 pt-10 shadow-[0_8px_40px_rgba(79,140,255,0.10)] [backface-visibility:hidden] [transform:rotateY(180deg)]">

      {/* STORY 배지 — absolute top-3 left-3 */}
      <StoryLabel storyNumber={storyNumber} onJump={onJump} />

      {/* 하트 — absolute top-3 right-3 */}
      {onToggleFavorite && (
        <button
          aria-label={isFavorited ? '즐겨찾기 해제' : '즐겨찾기 추가'}
          className="absolute right-3 top-3 z-10 rounded-full p-1.5 transition-colors hover:bg-[#F0F7FF]"
