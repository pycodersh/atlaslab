...
interface Props {
  /** 서버 컴포넌트가 존재 여부를 확인한 레시피 슬러그 */
  slug: string
}

export default function RecipeDetailClient({ slug }: Props) {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // getRecipeDetail 은 행이 없거나 조회 실패 시 throw 한다.
        // 잡지 않으면 "Loading recipe..." 상태로 영구히 멈춘다.
        setRecipe(null)
      } finally {
        setLoading(false)
      }
