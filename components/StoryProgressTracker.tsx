...
export function StoryProgressTracker({
  ...
  // Pattern Card note colors (same tokens)
  ...
  // Info icon color (matches Pattern Card: yellow when open, muted when closed)
  ...
  // Fixed column widths for pixel-perfect row alignment
  const DOTS_W   = 48  // px — dots area (both rows start at same x)
  const ACTION_W = 36  // px — check button / spacer
  ...
  return (
    <div style={{ padding: '0 16px 4px' }}>
      ...
      {/* ── Info button row (right-aligned, flush with STUDY RECORD header) ── */}
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <span style={{
            ...
            color: listeningCompleted ? accentOn : labelColor,
          }}>
            Listening · 1회
          </span>
          {/* Dots (fixed width = DOTS_W) */}
          <div style={{ width: DOTS_W, display: 'flex', alignItems: 'center', gap: 4 }}>
            {dot(listeningCompleted)}
          </div>
          {/* Action spacer (fixed width = ACTION_W) */}
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <span style={{
            flex: 1, fontSize: 12, fontWeight: 600,
            color: readingDone ? accentOn : labelColor,
          }}>
            Reading · 2회
          </span>
          {/* Dots (fixed width = DOTS_W, same x as Listening dot) */}
          <div style={{ width: DOTS_W, display: 'flex', alignItems: 'center', gap: 4 }}>
            {dot(readingCount >= 1)}
            {dot(readingCount >= 2)}
          </div>
