...
- **심각도:** High
- **환경:** Desktop Chrome / Mobile
- **경로:** `/patto/settings`
- **재현율:** 5/5 (항상 재현)
- **기대 결과:** 영어 설정 시 "Continue with Google" 등 영어로 표시
- **실제 결과:** "Google로 계속하기", "이메일로 계속하기", "카카오로 계속하기" — 언어 설정과 무관하게 한국어 고정
- **관련 파일:** `components/auth/AuthButtons.tsx` (추정)
- **추정 원인:** 소셜 로그인 버튼 텍스트가 i18n 훅 없이 하드코딩됨
- **수정 방향:** `useT()` 기반 번역 키 적용
...
- **경로:** `/patto/settings`
- **재현율:** 5/5 (항상 재현)
- **기대 결과:** 영어 설정 시 "Add PATTO to Home Screen" 등 영어로 표시
- **실제 결과:** "PATTO를 홈 화면에 추가", "더 빠르게 PATTO를 실행할 수 있어요.", "나중에", "추가하기" — 한국어 고정
- **관련 파일:** Settings 페이지 내 PWA 배너 컴포넌트
- **수정 방향:** 번역 키 추가 후 `useT()` 적용
...
| 13 | 한국어·영어 하드코딩 검사 | ⚠️ WARN | BUG-001 (High), BUG-002 (Medium) 발견 |
