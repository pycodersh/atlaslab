...
export default function PattoLayout({
  ...
  return (
    <div className={`antialiased ${jakartaSans.variable} ${baloo2.variable} ${playfair.variable} ${kalam.variable}`}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}><ThemeProvider><TrainerOrbProvider><TrainerProvider><PreferencesProvider><AuthProvider><OneSignalInit /><WelcomeCover /><PageTransition>{children}</PageTransition><TrainerOrb /></AuthProvider></PreferencesProvider></TrainerProvider></TrainerOrbProvider></ThemeProvider></div>
    </div>
  );
}
