...
import { TrainerProvider } from "@/components/GuideDock";
;
...
export default function PattoLayout({
  ...
  return (
    <div className={`antialiased ${jakartaSans.variable} ${baloo2.variable} ${playfair.variable} ${kalam.variable}`}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}><ThemeProvider><PreferencesProvider><AuthProvider><TrainerProvider><OneSignalInit /><WelcomeCover /><PageTransition>{children}</PageTransition><MainTabBar /></TrainerProvider></AuthProvider></PreferencesProvider></ThemeProvider></div>
    </div>
  );
}
