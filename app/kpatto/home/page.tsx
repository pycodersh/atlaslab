...
function buildContinueEp(epNum: number): ContinueEp {
  const num    = Math.max(1, Math.min(epNum, 100))
  const id     = `kp-ep-${String(num).padStart(3, '0')}`
  const static_ = ALL_STORIES.find(s => s.episode === num)
  return {
    num,
    id,
    title:         static_?.title ?? `Episode ${num}`,
    thumbnail_url: static_?.thumbnail_url
...
export default function KPattoHomePage() {
  ...
  const [continueEp,       setContinueEp]        = useState<ContinueEp>(buildContinueEp(1))
  ...
  useEffect(() => {
    ...
    // 다음 화: 신규 시스템에서 최대 완료 화 조회
    // (버그 수정: 지역 변수로 판정 — setState 비동기 우회)
    getMaxCompletedEpisode().then(maxDone => {
      const hasProgress = maxDone > 0 || hasOldRecs
      setIsReturning(hasProgress)
      // maxDone=0 이면 아직 완료 없음 → EP01 시작
      const nextNum = maxDone > 0 ? Math.min(maxDone + 1, 100) : 1
      setContinueEp(buildContinueEp(nextNum))
    })

    setHydrated(true)
  }, [])

  // ── Today's Expression (DB에서 날짜 기반 선택) ───────────────────────────
  useEffect(() => {
    if (subLoading) return  // 구독 상태 확인 후 풀 결정
  ...
  const epLabel = `EP${String(continueEp.num).padStart(2, '0')}`
    ...
    <div style={{ maxWidth: 480, margin: '0 auto' }}>
      ...
      {todayExpr && (
        <div style={{ padding: '20px 16px 0' }}>
          ...
          <Link href={`/kpatto/expressions/${todayExpr.id}`} style={{ textDecoration: 'none', display: 'block' }}>
            <div style={{
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={{ padding: '28px 16px 0', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <Link href="/kpatto/pre-course" style={{ textDecoration: 'none', display: 'block' }}>
          <div style={{ background: '#1A1A1A', borderRadius: 18, padding: '20px 20px' }}>
        ...
        <Link href="/kpatto/story" style={{ textDecoration: 'none', display: 'block' }}>
          <div style={{
            border: `1.5px solid ${DIV}`, borderRadius: 18, padding: '18px 20px',
            ...
            <div>
              ...
              <div style={{ fontSize: 13, color: T2 }}>Jump to Episode 1 →</div>
            </div>
            <div style={{
              width: 56, height: 56, borderRadius: 12, overflow: 'hidden',
              flexShrink: 0, background: '#F7F7F7', position: 'relative',
            }}>
              <Image src="/kpatto/banners/ep1.png" alt="EP01" fill style={{ objectFit: 'cover' }} sizes="56px" />
            </div>
          </div>
        </Link>
      </div>
      ...
      <div style={{
        margin: '24px 16px 0',
        border: `1px solid ${DIV}`, borderRadius: 16, padding: '16px 20px',
        display: 'flex', justifyContent: 'space-between',
      }}>
        {[{ num: '100', label: 'stories' }, { num: '300+', label: 'expressions' }, { num: '10', label: 'free episodes' }].map(({ num, label }) => (
          <div key={label} style={{ textAlign: 'center', flex: 1 }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: ACCENT, letterSpacing: '-0.04em' }}>{num}</div>
        ...
        <Link href="/kpatto/expressions" style={{ textDecoration: 'none', display: 'block' }}>
          <div style={{
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: T2 }}>Essential expressions with examples</div>
            </div>
